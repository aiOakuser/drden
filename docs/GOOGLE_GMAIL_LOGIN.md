...
|-------------|-------------------------|
| **Production** | `https://designrden.com/auth/complete/google-oauth2/` |
| **Production (www)** | `https://www.designrden.com/auth/complete/google-oauth2/` |
...
- If you don’t have a project for designrden, click **Create project**, name it (e.g. “designrden”), then select it.

### 3. OAuth consent screen

- Go to **APIs & Services** → **OAuth consent screen**.
- If not set up: choose **External** (or Internal for Workspace), then fill:
  - **App name:** e.g. designrden
...
- **Name:** e.g. “designrden Web” (any label).
...
**Production (required for designrden.com):**

```
https://designrden.com/auth/complete/google-oauth2/
```

If you also use `www`:

```
https://www.designrden.com/auth/complete/google-oauth2/
```
...
- `social_django.urls` under `path("auth/", ...)` in `drden/urls.py`
- `SOCIAL_AUTH_GOOGLE_OAUTH2_KEY` / `_SECRET` from `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` in `drden/settings.py`
...
- [ ] **Authorized redirect URIs** include exactly:  
  `https://designrden.com/auth/complete/google-oauth2/`  
  (and `https://www.designrden.com/auth/complete/google-oauth2/` if you use www).
