...
def _google_oauth_redirect_uri(request: HttpRequest) -> str:
    ...
    from drden.social_strategy import CanonicalDjangoStrategy
...
def drden_instagram_url(request: HttpRequest) -> dict:
    """Official Global Designer Hub Instagram profile URL (override via drden_INSTAGRAM_URL)."""
    return {"drden_instagram_url": getattr(settings, "drden_INSTAGRAM_URL", "") or ""}
...
def mobile_app_context(request: HttpRequest) -> dict:
    ...
    """
    app_name = (getattr(settings, "IOS_APP_NAME", "designrden") or "designrden").strip() or "designrden"
