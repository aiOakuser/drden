...
@override_settings(
    ...
    CANONICAL_HOST="designrden.com",
    CANONICAL_REDIRECT_HOSTS=["www.designrden.com"],
...
class CanonicalDomainRedirectMiddlewareTests(TestCase):
    def test_get_request_redirects_with_301(self):
        response = self.client.get(
            reverse("login"),
            HTTP_HOST="www.designrden.com",
            follow=False,
        )
        self.assertEqual(response.status_code, 301)
        self.assertEqual(
            response.headers.get("Location"),
            "https://designrden.com" + reverse("login"),
        )
...
class PasswordResetFlowTests(TestCase):
    ...
    def test_incomplete_smtp_credentials_use_console_in_debug(self):
        from drden.settings import resolve_email_backend
...
@override_settings(
    ...
    CANONICAL_DOMAIN_REDIRECT_ENABLED=True,
    CANONICAL_REDIRECT_SCHEME="https",
    ALLOWED_HOSTS=["testserver", "designrden.com", "www.designrden.com"],
)
class CanonicalDomainRedirectTests(TestCase):
    def test_www_redirects_to_canonical(self):
        response = self.client.get("/", HTTP_HOST="www.designrden.com")
        self.assertEqual(response.status_code, 301)
        self.assertEqual(response.headers.get("Location"), "https://designrden.com/")

    def test_canonical_host_serves_homepage(self):
        response = self.client.get("/", HTTP_HOST="designrden.com")
        self.assertEqual(response.status_code, 200)
...
class ReportProblemViewTests(TransactionTestCase):
    ...
    def test_anonymous_submission_creates_report_and_sends_email(self):
        ...
        payload = {
            ...
            "page_url": "https://designrden.com/dashboard/",
        }
        ...
        self.assertEqual(mail.outbox[0].subject, "[designrden] New problem report")
...
class StudentPageViewTests(TestCase):
    ...
    def test_student_page_supports_url_without_trailing_slash(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "designrden Student")
