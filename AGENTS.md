...
The `.env` file must contain at minimum `DEBUG=True` and `DATABASE_URL=sqlite:///db.sqlite3` for local dev (SQLite). The settings file (`drden/settings.py`) will raise a `RuntimeError` if no database configuration is found and you are not running tests.
...
```
ruff check --select=E,F,I --ignore=E501 drden/ designer_portfolio/ marketing/ sitebuilder/ ai_chat/
```
